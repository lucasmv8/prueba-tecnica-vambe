...
function dryRunBatch(clients: BatchInput[]): AnalysisResult[] {
  ...
  const outputTokensEst = clients.length * 90;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!(error instanceof Anthropic.APIError)) {
        throw error instanceof Error ? error : new Error("Error desconocido en Claude API");
      }
      ...
      throw new Error(`[${status}] ${error.message || "Error desconocido en Claude API"}`);
    }
  }
