  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className="bg-[#161616] border border-[#2A2A2A] rounded-2xl shadow-2xl w-full max-w-[540px] pointer-events-auto flex flex-col" style={{ maxHeight: "85vh" }}>
          {/* Header */}
          <div className="px-5 py-4 border-b border-[#2A2A2A] flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <Mail size={13} className="text-[#60A5FA]" />
                <span className="text-sm font-semibold text-white">Redactar mensaje</span>
              </div>
              <p className="text-xs text-[#A0A0A0]">Para: {alert.nombre} · {alert.correo}</p>
            </div>
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg text-[#A0A0A0] hover:text-white hover:bg-[#2A2A2A] transition-colors cursor-pointer shrink-0"
            >
              <X size={14} />
            </button>
          </div>
          ...
          {sent ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 p-8">
              ...
              <p className="text-sm font-medium text-white">Mensaje listo para enviar</p>
              <p className="text-xs text-[#A0A0A0] text-center max-w-[280px]">
                Esta es una simulación. En producción el mensaje se enviaría a{" "}
                <span className="text-white">{alert.correo}</span>.
              </p>
              <button
                onClick={onClose}
                className="mt-2 px-4 py-2 rounded-lg text-xs font-medium bg-[#1E1E1E] text-[#A0A0A0] border border-[#2A2A2A] hover:text-white hover:border-[#3A3A3A] transition-colors cursor-pointer"
              >
                Cerrar
              </button>
            </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#111111] border border-[#2A2A2A] rounded-lg px-3 py-2 text-xs text-white placeholder:text-[#505050] focus:outline-none focus:border-[#2563EB]/60 transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={8}
                    className="w-full bg-[#111111] border border-[#2A2A2A] rounded-lg px-3 py-2 text-xs text-white placeholder:text-[#505050] focus:outline-none focus:border-[#2563EB]/60 transition-colors resize-none leading-relaxed"
                  />
                </div>
                ...
                <div className="border-t border-[#2A2A2A] pt-3">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-[#A0A0A0] uppercase tracking-wider font-medium">Versión generada con IA</span>
                    </div>
                    <button
                  ...
                  {aiLoading && !aiResult && (
                    <div className="bg-[#111111] border border-[#A855F7]/15 rounded-lg p-3 space-y-2 animate-pulse">
                      <div className="h-2.5 bg-[#2A2A2A] rounded w-2/3" />
                      <div className="h-2 bg-[#2A2A2A] rounded w-full" />
                      <div className="h-2 bg-[#2A2A2A] rounded w-5/6" />
                      <div className="h-2 bg-[#2A2A2A] rounded w-4/6" />
                    </div>
                  )}

                  {aiResult && (
                    <div className="bg-[#111111] border border-[#A855F7]/20 rounded-lg p-3 space-y-2">
                      <p className="text-[10px] text-[#A855F7]/70 font-medium">{aiResult.subject}</p>
                      <p className="text-[11px] text-[#C0C0C0] leading-relaxed whitespace-pre-wrap">{aiResult.body}</p>
                      <button
                        onClick={handleUseAI}
              ...
              <div className="px-5 py-4 border-t border-[#2A2A2A] flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={onClose}
                    className="px-3 py-1.5 rounded-lg text-xs font-medium bg-[#1E1E1E] text-[#A0A0A0] border border-[#2A2A2A] hover:text-white hover:border-[#3A3A3A] transition-colors cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
  ...
  return (
    <div className={`relative group min-w-0 truncate ${className ?? ""}`}>
      ...
      <div className="pointer-events-none absolute bottom-full left-0 mb-1.5 z-50 hidden group-hover:block">
        <div className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-lg px-2.5 py-1.5 text-[11px] text-[#E0E0E0] shadow-xl whitespace-normal max-w-[340px] leading-relaxed">
          {text}
        </div>
      </div>
    </div>
...
function AlertRow({ alert, onContact }: { alert: AlertEntry; onContact: (a: AlertEntry) => void }) {
  ...
  return (
    <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border ${
      isWarning ? "bg-amber-500/5 border-amber-500/15" : "bg-[#1A1A1A] border-[#2A2A2A]"
    }`}>
      {isWarning
      ...
      {/* Name + vendedor + industria */}
      <div className="shrink-0 min-w-[160px]">
        <span className="text-xs font-medium text-white">{alert.nombre}</span>
        <span className="text-[11px] text-[#505050] ml-1.5">· {alert.vendedor}</span>
        {alert.industria && (
          <span className="ml-1.5 text-[10px] text-[#6B7280] bg-[#222] border border-[#2A2A2A] px-1.5 py-0.5 rounded-full">
            {capitalizeFirst(alert.industria)}
          </span>
        )}
      </div>

      {/* Pain point + próxima acción */}
      <div className="flex-1 min-w-0 flex items-center gap-2 overflow-hidden">
        {alert.painPoint && (
          <TruncatedText text={alert.painPoint} className="text-[11px] text-[#A0A0A0] shrink-0" />
        )}
      ...
      {/* Tag + button */}
      <span className={`text-[10px] font-medium shrink-0 ${isWarning ? "text-amber-400" : "text-[#6B7280]"}`}>
        {alert.etiqueta}
      </span>
      <button
    ...
    <>
      ...
      <div className="bg-[#161616] border border-[#2A2A2A] rounded-xl">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-5 py-4 cursor-pointer hover:bg-[#1A1A1A] transition-colors rounded-xl"
        >
          <AlertTriangle size={13} className="text-amber-400 shrink-0" />
          <h3 className="text-sm font-medium text-[#A0A0A0] uppercase tracking-wider">
            Señales de Atención
          </h3>
            ...
            {warnings.length > 0 && (
              <div className="space-y-1.5">
                <div className="flex items-baseline gap-2">
                  <p className="text-[10px] text-amber-400/70 uppercase tracking-wider font-medium">
