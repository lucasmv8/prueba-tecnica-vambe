...
import type { MetricsData, BarChartEntry, LineChartEntry, DuplicateEmailGroup, PainPointEntry, FunnelEntry } from "../types";
...
export async function getMetrics(): Promise<MetricsData> {
  ...
  const closed = allClients.filter((c: any) => c.closed).length;
  const pending = allClients.filter((c: any) => !c.analyzedAt).length;

  // ── Leads Calificados ──────────────────────────────────────────────────────
  // Leads abiertos, analizados, que tienen urgencia alta o ya están evaluando/listos
  ...
  // Top vendedor by close rate (min 3 deals)
  ...
  // Top industria by close rate (min 2 deals)
