...
export function ChartsSection({ data, view = "all" }: Props) {
  const industriaData = data.byIndustria
    .map((d) => ({ ...d, name: capitalizeFirst(d.name) }))
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
      ...
      {showAnalisis && <div className="bg-[#161616] border border-[#2A2A2A] rounded-xl p-5">
        <div className="flex items-baseline justify-between mb-4">
          <h3 className="text-base font-semibold text-white">Close Rate por Industria</h3>
          <span className="text-xs text-[#505050]">% cierre</span>
        </div>
        <div className="space-y-3">
          {industriaData.map(({ name, closeRate }) => (
            <div key={name} className="flex items-center gap-3">
              <span className="text-xs text-[#A0A0A0] w-24 shrink-0 truncate">{name}</span>
